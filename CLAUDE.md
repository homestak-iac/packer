...
│   └── */.cache-key      # Build cache keys (generated by build.sh)
...
`build.sh` skips rebuilding when the built image is still valid. Cache validity is checked via a composite key:
...
```bash
./build.sh debian-12          # Uses cache if valid
./build.sh --force debian-12  # Always rebuild
```
